...
export default function App() {
    ...
    /* AURORA WAVE FIELD — animated canvas background */
    ...
    useEffect(() => {
        ...
        const skyLines = Array.from({ length: NUM_SKY_LINES }).map((_, i) => {
            ...
            return {
                ...
                depth: p,
                colorStops:
                    p < 0.5 ? ["#3b82f6", "#8b5cf6"] : ["#8b5cf6", "#d946ef"],
            };
        });
        ...
        function drawWaveLine(cfg, t, w, h) {
            ...
            ctx.shadowColor =
                cfg.colorStops[Math.floor(cfg.colorStops.length / 2)];
            const gradient = ctx.createLinearGradient(0, 0, w, 0);
            const n = cfg.colorStops.length;
            cfg.colorStops.forEach((c, idx) =>
                gradient.addColorStop(n === 1 ? 0 : idx / (n - 1), c),
            );
            ...
            for (let x = -step; x <= w + step; x += step) {
                const y =
                    cfg.baseYRatio * h +
                    Math.sin(x * cfg.freqA + t * cfg.speed + cfg.phase) *
                        cfg.ampA +
                    Math.sin(
                        x * cfg.freqB - t * cfg.speed * 0.7 + cfg.phase * 1.3,
                    ) *
                        cfg.ampB +
