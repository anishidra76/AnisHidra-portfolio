...
export default function Management() {
    ...
    const fetchData = async () => {
        try {
            ...
            const [clientsData, incomeData, expenseData] = await Promise.all([
                request(API.clients),
    ...
    const calculations = useMemo(() => {
        ...
        const clientExpenses = clients.reduce(
            (sum, client) => sum + numberValue(client.expenses),
            0,
        );

        const externalGrossIncome = externalIncome.reduce(
            (sum, item) => sum + numberValue(item.amount),
            0,
        );

        const externalIncomeExpenses = externalIncome.reduce(
            (sum, item) => sum + numberValue(item.expenses),
            0,
        );

        const externalExpenses = externalExpense.reduce(
            (sum, item) => sum + numberValue(item.expenses),
            0,
        );

        const totalIncome = clientIncome + externalGrossIncome;

        const totalExpenses =
            clientExpenses + externalIncomeExpenses + externalExpenses;
    ...
    }, [clients, externalIncome, externalExpense]);
    ...
    const convertedBalance = {
        EUR: rates.EUR !== null ? calculations.balance * rates.EUR : null,

        GBP: rates.GBP !== null ? calculations.balance * rates.GBP : null,

        CHF: rates.CHF !== null ? calculations.balance * rates.CHF : null,

        DZD: rates.DZD !== null ? calculations.balance * rates.DZD : null,
    };
    ...
    const chartData = useMemo(() => {
        ...
        const addIncome = (date, amount) => {
            ...
            months[key].income += numberValue(amount);
        };
        ...
        const addExpense = (date, amount) => {
            ...
            months[key].expenses += numberValue(amount);
        };

        clients.forEach((client) => {
            addIncome(client.start, client.amount);

            addExpense(client.start, client.expenses);
        });

        externalIncome.forEach((item) => {
            addIncome(item.date, item.amount);

            addExpense(item.date, item.expenses);
        });
    ...
    const saveIncome = async (e) => {
        ...
        try {
            ...
            if (editingIncome) {
                await request(`${API.externalIncome}${editingIncome.id}/`, {
                    method: "PATCH",
                    body: JSON.stringify(body),
                });
            } else {
                await request(API.externalIncome, {
                    method: "POST",
                    body: JSON.stringify(body),
                });
            }
            ...
            alert("Unable to save external income.");
        }
    };
    ...
    const deleteIncome = async (id) => {
        ...
        try {
            await request(`${API.externalIncome}${id}/`, {
                method: "DELETE",
            });
            ...
            alert("Unable to delete external income.");
        }
    };
    ...
    const saveExpense = async (e) => {
        ...
        try {
            const body = {
                ...expenseForm,
                expenses: Number(expenseForm.expenses),
            };

            if (editingExpense) {
                await request(`${API.externalExpense}${editingExpense.id}/`, {
                    method: "PATCH",
                    body: JSON.stringify(body),
                });
            } else {
                await request(API.externalExpense, {
                    method: "POST",
                    body: JSON.stringify(body),
                });
            }
            ...
            alert("Unable to save external expense.");
        }
    };
    ...
    const deleteExpense = async (id) => {
        ...
        try {
            await request(`${API.externalExpense}${id}/`, {
                method: "DELETE",
            });
            ...
            alert("Unable to delete external expense.");
        }
    };
    ...
    if (loading) {
        return <div className={styles.loading}>Loading Management...</div>;
    }
        ...
        <div className={styles.management}>
            ...
            <div className={styles.pageHeader}>
                <div>
                    ...
                    <p>Manage clients, incomes, expenses and profits.</p>
                </div>
            </div>
            ...
            <section className={styles.balanceSection}>
                ...
                >
                    {calculations.balance >= 0 ? "+ " : "- "}${" "}
                    {formatNumber(Math.abs(calculations.balance))}
                </div>
                ...
                <div className={styles.currencies}>
                    <Currency
                        ...
                        positive={calculations.balance >= 0}
                    />
                ...
                <div className={styles.balanceSummary}>
                    <div>
                        ...
                        <strong className={styles.income}>
                            {incomeDisplay(calculations.totalIncome)}
                        </strong>
                    </div>
                    ...
                    <div>
                        ...
                        <strong className={styles.expense}>
                            {expenseDisplay(calculations.totalExpenses)}
                        </strong>
                    </div>
                </div>
            </section>
            ...
            <section className={styles.summaryGrid}>
                <SummaryCard
                    ...
                    value={calculations.clientIncome}
                    type="income"
                />

                <SummaryCard
                    title="Client Expenses"
                    value={calculations.clientExpenses}
                    type="expense"
                />

                <SummaryCard
                    title="External Gross Income"
                    value={calculations.externalGrossIncome}
                    type="income"
                />
            ...
            <section className={styles.section}>
                ...
                <div className={styles.tableWrapper}>
                    <table>
                        ...
                        <tbody>
                            {clients.length === 0 ? (
                                <EmptyRow colSpan={12} text="No clients yet." />
                            ) : (
                                clients.map((client) => (
                                    <tr key={client.id}>
                                        <td>
                                        ...
                                        <td>{formatDate(client.start)}</td>

                                        <td>{formatDate(client.end)}</td>

                                        <td className={styles.income}>
                                            {incomeDisplay(client.amount)}
                                        </td>

                                        <td className={styles.expense}>
                                            {expenseDisplay(client.expenses)}
                                        </td>

                                        <td
                                            className={
                                                numberValue(client.total) >= 0
                                                    ? styles.income
                                                    : styles.expense
                                            }
                                        >
                                            {numberValue(client.total) >= 0
                                                ? incomeDisplay(client.total)
                                                : expenseDisplay(
                                                      Math.abs(
                                                          numberValue(
                                                              client.total,
                                        ...
                                        <td>
                                            <span
                                                className={`${styles.status} ${styles[client.status]}`}
                                            >
                                                {client.status}
                                            </span>
                                        </td>
                                ...
                                {externalIncome.length === 0 ? (
                                    ...
                                    externalIncome.map((item) => (
                                        <tr key={item.id}>
                                            ...
                                            <td className={styles.income}>
                                                {incomeDisplay(item.amount)}
                                            </td>

                                            <td className={styles.expense}>
                                                {expenseDisplay(item.expenses)}
                                            </td>
...
function Currency({ name, value, positive }) {
    return (
        <div className={styles.currency}>
            ...
            <strong className={positive ? styles.income : styles.expense}>
                {value === null
                    ? "Loading..."
...
function SummaryCard({ title, value, type }) {
    return (
        <div className={styles.summaryCard}>
            ...
            <strong
                className={type === "income" ? styles.income : styles.expense}
            >
                {type === "income"
                    ? incomeDisplay(value)
