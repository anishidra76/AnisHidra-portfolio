...
function Messages() {
  ...
  const markAsRead = async (message) => {
    ...
    try {
      ...
      setMessages((prevMessage) => prevMessage.map((item) => item.id === message.id ? { ...item, is_read: true} : item));
    } catch(error) {
      console.error(error);
    }
  };
  ...
  const deleteMessage = async(id) => {
    const confirmed = window.confirm("Are you want to delete this message?");
    if(!confirmed) return;
    try {
      const response = await fetch(
