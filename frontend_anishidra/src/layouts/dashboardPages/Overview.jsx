...
const API = {
    ...
    // Other APIs
    messages: `${API_URL}/api/contact/contactmessages/`,
...
export default function Overview() {
    ...
    useEffect(() => {
        const fetchManagementData = async () => {
            try {
                const [clientsData, incomeData, expenseData] =
                    await Promise.all([
                        request(API.clients),
    ...
    useEffect(() => {
        const fetchVisitors = async () => {
            try {
                const data = await request(API.visitors);
    ...
    const calculations = useMemo(() => {
        ...
        const clientIncome = clients.reduce(
            (sum, client) => sum + numberValue(client.amount),
            0,
        );
    ...
    const visitorsThisMonth = visitors.filter((visitor) => {
        const date = visitor.visited_at || visitor.createdAt || visitor.date;
        ...
        return (
            visitorDate.getMonth() === currentMonth.getMonth() &&
            visitorDate.getFullYear() === currentMonth.getFullYear()
        );
    }).length;
    ...
    const visitorChartData = useMemo(() => {
        ...
        visitors.forEach((visitor) => {
            const date =
                visitor.visited_at || visitor.createdAt || visitor.date;
    ...
    if (loading) {
        return (
            <div className={styles.overview}>
                <div className={styles.empty}>Loading Overview...</div>
            </div>
        );
    }
        ...
        <div className={styles.overview}>
            ...
            <div className={styles.stats}>
                ...
                <div className={styles.card}>
                    ...
                    <strong>{totalVisitors.toLocaleString("en-US")}</strong>

                    <small>Visitors Total</small>
                </div>
                ...
                <div className={styles.card}>
                    ...
                    <strong>{visitorsThisMonth.toLocaleString("en-US")}</strong>
                ...
                <div className={styles.card}>
                    ...
                    <strong className={styles.income}>
                        + $ {formatMoney(calculations.totalIncome)}
                    </strong>

                    <small>Total Gross Income</small>
                </div>
                ...
                <div className={styles.card}>
                    ...
                    >
                        {calculations.balance >= 0 ? "+ " : "- "}${" "}
                        {formatMoney(Math.abs(calculations.balance))}
                    </strong>

                    <small>Net Profit Remaining</small>
                </div>
            </div>
