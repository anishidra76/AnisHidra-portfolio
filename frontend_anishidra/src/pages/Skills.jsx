        ...
        categories.forEach((cat) => {
            ...
            groups.forEach((group) => {
                ...
                items.forEach((item) => {
                    ...
                    const status = getLevelLabelAndColor(level);
                    fill.style.width = level + "%";
                    fill.style.backgroundColor = status.color;
                    grpTotal += level;
                });
                if (items.length === 0) return;
                const grpAvg = grpTotal / items.length;
                const grpStatus = getLevelLabelAndColor(grpAvg);
                const grpLvlEl = group.querySelector(".grp-lvl");
                if (grpLvlEl) {
                    grpLvlEl.textContent = grpStatus.label;
                    grpLvlEl.style.color = grpStatus.color;
                }
                catTotal += grpAvg;
                catCount++;
            });
            ...
            const catStatus = getLevelLabelAndColor(catAvg);
            const badge = cat.querySelector(".overall-badge");
            if (badge) {
                badge.textContent = `Overall: ${catStatus.label} (${Math.round(catAvg)}%)`;

                badge.style.border = `1px solid ${catStatus.color}`;

                badge.style.color = catStatus.color;
            }
        });
    }, [domains]);
